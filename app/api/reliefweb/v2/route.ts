...
export async function POST(request: NextRequest) {
  try {
    ...
    const baseEndpoint = useStream
      ? `${BACKEND_URL}/api/jobs/reliefweb/v2/stream`
      : `${BACKEND_URL}/api/jobs/reliefweb/v2`;
    ...
    console.error("ReliefWeb v2 API error:", error);
    return NextResponse.json(
      {
        error: "Failed to search ReliefWeb v2 jobs",
